...
This is the contract for 0xCONTRACT_NAME NFTs! 
...
import MutableMetadata from 0xNIFTORY
import MutableMetadataTemplate from 0xNIFTORY
import MutableMetadataSet from 0xNIFTORY
import MutableMetadataSetManager from 0xNIFTORY
import MetadataViewsManager from 0xNIFTORY

import NiftoryNonFungibleToken from 0xNIFTORY
import NiftoryNFTRegistry from 0xNIFTORY

import NiftoryMetadataViewsResolvers from 0xNIFTORY

import NiftoryNonFungibleTokenProxy from 0xNIFTORY

pub contract 0xCONTRACT_NAME: NonFungibleToken {
    ...
    NiftoryNonFungibleToken.NFTPublic
  {
    ...
    pub fun contract(): &{NiftoryNonFungibleToken.ManagerPublic} {
      return 0xCONTRACT_NAME.contract()
    }
    ...
    init(setId: Int, templateId: Int, serial: UInt64) {
      self.id = 0xCONTRACT_NAME.totalSupply
      0xCONTRACT_NAME.totalSupply =
        0xCONTRACT_NAME.totalSupply + 1
      self.setId = setId
    ...
    NiftoryNonFungibleToken.CollectionPrivate
  {
    ...
    pub fun deposit(token: @NonFungibleToken.NFT) {
      let token <- token as! @0xCONTRACT_NAME.NFT
    ...
    pub fun depositBulk(tokens: @[NonFungibleToken.NFT]) {
      while tokens.length > 0 {
        let token <- tokens.removeLast() as! @0xCONTRACT_NAME.NFT
    ...
    NiftoryNonFungibleToken.ManagerPrivate 
  {
    ...
    pub fun metadata(): AnyStruct? {
      return 0xCONTRACT_NAME.metadata
    }
      ...
      &MutableMetadataSetManager.Manager{MutableMetadataSetManager.Public}
    {
      return NiftoryNFTRegistry
        .getSetManagerPublic(
          0xCONTRACT_NAME.REGISTRY_ADDRESS,
          0xCONTRACT_NAME.REGISTRY_BRAND
        )
    }
      ...
      return NiftoryNFTRegistry
        .buildNFTCollectionData(
          ...
          (fun (): @NonFungibleToken.Collection {
            return <-0xCONTRACT_NAME.createEmptyCollection()
          })
        )
    }
    ...
    pub fun modifyContractMetadata(): auth &AnyStruct {
      return &0xCONTRACT_NAME.metadata as auth &AnyStruct
    }

    pub fun replaceContractMetadata(_ metadata: AnyStruct?) {
      0xCONTRACT_NAME.metadata = metadata
    }
  ...
  init(
    ...
    self.REGISTRY_ADDRESS = 0xNFT_REGISTRY
    ...
    self
      ...
      .save<@MutableMetadataSetManager.Manager>(
        <-MutableMetadataSetManager.create(
          name: "0xCONTRACT_NAME",
          description: "The set manager for 0xCONTRACT_NAME."
        ),
    ...
    let contractName = "0xCONTRACT_NAME"
